...
dashboard_path <- "."  # Donde está el Rmd
...
# ---- Descomprimir imágenes DIRECTO al directorio del Rmd ----
...
```{r}
datos_OWID <- read_csv("required_data/datasets/KEY_INDICATORS.csv") %>% as_tibble()
datos_OWID
```
...
```{r}
insercion <- read_csv("required_data/datasets/INSERCION_LABORAL.csv") %>% as_tibble() %>% select(-"...1" )
insercion
```
...
```{r}
empleo_municipios_IC <- read_csv("required_data/datasets/EMPLEO_MUNICIPIOS.csv") %>% as_tibble() %>% select(-"...1")
...
```{r}
contratos_IC <- read_csv("required_data/datasets/CONTRATOS_2018_2024.csv") %>% as_tibble() %>% select(-año, -mes)
contratos_IC
```
...
```{r}
EMPLEO <- read_csv("required_data/datasets/EMPLEO.csv")
...
TASA_EMPLEO <- read.csv("required_data/datasets/TASA_EMPLEO.csv") %>%
  as_tibble()
...
KEY_INDICATORS <- read.csv("required_data/datasets/KEY_INDICATORS.csv") %>%
  as_tibble()

CONTRATOS_2018_2024 <- read_csv("required_data/datasets/CONTRATOS_2018_2024.csv") %>% as_tibble()

INSERCION_LABORAL <- read_csv("required_data/datasets/INSERCION_LABORAL.csv") %>% as_tibble()

EMPLEO_MUNICIPIOS <- read_csv("required_data/datasets/EMPLEO_MUNICIPIOS.csv") %>% as_tibble() %>% select(-geom) %>%
...
```{r}
geoj <- geojson_read("required_data/geos/geoj_0.geojson", what = "sp")
...
Además, el repositorio ha sido configurado para publicar la versión compilada de la memoria en formato HTML mediante *GitHub Pages*, funcionando como una versión estable y accesible del documento. A diferencia del dashboard interactivo —que requiere un servidor de cálculo activo en R—, esta publicación estática permite que **cualquier persona con acceso al enlace** pueda consultar íntegramente el informe, incluyendo los análisis y las visualizaciones interactivas, desde dispositivos móviles o de escritorio. De este modo, se garantiza la accesibilidad, conservación y difusión universal de los resultados del proyecto.
...
p <- SeguimientoActividadesProyecto%>%
  ...
  geom_bar(stat = "identity") +
  geom_text(aes(label=HORAS),size=3,colour = "blue",nudge_y = 1.5)+ 
  theme(axis.text.x = element_text(angle = 45,hjust=1))+
...
p <- SeguimientoActividadesProyecto%>%
  ...
  geom_bar(stat = "identity") +
  geom_text(aes(label=HORAS),size=3,colour = "blue",nudge_y = 0.5)+ 
  theme(axis.text.x = element_text(angle = 45,hjust=1))+
