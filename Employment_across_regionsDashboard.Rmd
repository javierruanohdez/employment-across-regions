...
```{r}
EMPLEO <- readr::read_csv("EMPLEO.csv")
...
TASA_EMPLEO <- readr::read_csv("TASA_EMPLEO.csv") %>%
  as_tibble()
...
KEY_INDICATORS <- readr::read_csv("KEY_INDICATORS.csv") %>%
  as_tibble()
...
geoj_0 <- geojson_read("geoj_0.geojson",  what = "sp")
geoj_1 <- geojson_read("geoj_1.geojson",  what = "sp")
geoj_2 <- geojson_read("geoj_2.geojson",  what = "sp")
geoj_3 <- geojson_read("geoj_3.geojson",  what = "sp")

geoj <- list(geoj_0, geoj_1, geoj_2, geoj_3)

coord_0 <- readr::read_csv("eu_nuts0_coords.csv") %>% as_tibble()
coord_1 <- readr::read_csv("eu_nuts1_coords.csv") %>% as_tibble() %>% mutate(Latitude = as.numeric(Latitude))
coord_2 <- readr::read_csv("eu_nuts2_coords.csv") %>% as_tibble() %>% mutate(Latitude = as.numeric(Latitude))
coord_3 <- readr::read_csv("eu_nuts3_coords.csv") %>% as_tibble() %>% mutate(Latitude = as.numeric(Latitude), Longitude = as.numeric(Longitude))
...
CONTRATOS_2018_2024 <- readr::read_csv("CONTRATOS_2018_2024.csv") %>% as_tibble()

INSERCION_LABORAL <- readr::read_csv("INSERCION_LABORAL.csv") %>% as_tibble()

EMPLEO_MUNICIPIOS <- readr::read_csv("EMPLEO_MUNICIPIOS.csv") %>% as_tibble() %>% select(-geom) %>%
...
tags$div(
  ...
  # Breve descripción
  tags$div(
    "Explora la evolución del empleo en distintas regiones de Europa y España, incluyendo un enfoque en las Islas Canarias. Analiza indicadores clave, participación sectorial, brechas de género y tendencias por edad en un entorno visual e interactivo.",
    style = "max-width: 900px; font-size: 1.1em; color: #34495E; line-height: 1.5em; animation: fadeIn 2s; margin-bottom: 5px;"
  ),

  # Sección de herramientas (3 cajas en una misma línea)
  tags$div(
    style = "
      display: flex;
  ...
  tags$div(
    ...
    tags$div(
      ...
      ",
      tags$h3("📚 Contenidos", style="color: #2E86C1; font-size: 1.3em; font-weight: 800; margin-bottom: 15px;"),
      tags$ul(
        style="text-align: left; list-style-type: square; padding-left: 20px; font-size: 1em; line-height: 1.5em;",
        tags$li("Evolución del empleo regional y nacional"),
        tags$li("Participación sectorial según NACE Rev. 2"),
        tags$li("Brecha de género y análisis por edad"),
        tags$li("Series temporales y predicciones ARIMA"),
        tags$li("Mapas interactivos e indicadores clave"),
        tags$li("Análisis de correlación y componentes principales"),
        tags$li("Indicadores laborales específicos de Canarias"),
      )
    ),
    ...
    tags$div(
      style = "
        ...
        transition: transform 0.3s;
      ",
      tags$h3("🎯 Objetivos", style="color: #2E86C1; font-size: 1.3em; font-weight: 800; margin-bottom: 15px;"),
      tags$ul(
        style="text-align: left; list-style-type: square; padding-left: 20px; font-size: 1em; line-height: 1.5em;",
        tags$li("Visualizar tendencias de empleo a nivel regional y por sector"),
        tags$li("Identificar brechas de género y diferencias por edad"),
        tags$li("Facilitar comparación entre regiones y países"),
        tags$li("Analizar correlaciones entre indicadores económicos"),
        tags$li("Proyectar tendencias futuras mediante modelos ARIMA")
      )
    )
  ),

  # Plataformas de datos EN HORIZONTAL - SUBIDA MÁS ARRIBA
  tags$div(
    style = "
      background: white;
      border-radius: 15px;
      ...
      margin-bottom: 15px;
    ",
    tags$h3("📁 Fuentes de datos", style="color: #2E86C1; font-size: 1.3em; font-weight: 800; margin-bottom: 20px; text-align: center;"),
    tags$div(
      ...
      # 3. OBECAN (Con Hipervínculo - Reemplazar la URL de ejemplo)
      tags$div(
        style = "display: flex; justify-content: center; flex: 1;",
        tags$a(
      ...
      # 4. OWID (Con Hipervínculo - Reemplazar la URL de ejemplo)
      tags$div(
        style = "display: flex; justify-content: center; flex: 1;",
        tags$a(
  ...
  tags$div(
    ...
    ",
    "🚀 Para explorar el dashboard, selecciona la pestaña ",
...
div(
  ...
  div(
  style = "display: flex; flex-direction: column;",
   tags$label("Región", style = "margin-bottom: 3px;"),
    selectInput(
      "Region_a", 
      label = NULL,
  ...
  div(
    ...
    radioButtons(
      ...
      choices = c(
        ...
        "Per cápita" = "emp_per_working_age"
      ),
      selected = "employment",
      inline = TRUE
    )
...
# Crear gráfico interactivo directamente con plotly
p <- plot_ly()

# Añadir un segmento por cada fila
...
p <- layout(
  ...
  xaxis = list(
    title = list(text = ""),  # Quitamos el título del eje X
    tickmode = "array",
  ...
  tb <- tb %>% 
    ...
    mutate(
      legend_label = case_when(
        sector == "Primario" ~ "Primario (Agric., ganad., pesca, minería)",
        sector == "Secundario" ~ "Secundario (Industria, construcción)",
    ...
    tb <- inputs.processing.empleo_overview() %>%
          filter(TIME_PERIOD == ifelse(input$Region_a == "Norway", 2021, 2022),
                 !nace_r2.name %in% c("Total actividades", "Finanzas/Inmob./Profesionales/Administración"))

  
  jerarquia <- list(
    "Comer./Transp./Hostel." = "Comercio/Transporte/Hostelería/Info-Com",
    "Información/Comunic."  = "Comercio/Transporte/Hostelería/Info-Com",
  ...
  div(
    ...
    div(
      ...
      div(
        ...
        tags$label("Región", style = "margin-bottom: 3px; font-size: 0.85em;"),
        selectInput(
          "Region", 
          label = NULL,
    ...
    div(
      ...
      div(
        ...
        selectInput(
          "StaSex", 
          label = NULL,
          choices = c(setNames(unique(EMPLEO$wstatus), c("Total", "Asalariados", "Autónomos")), "Ambos"),
          selected = "EMP",
          width = "100%"
        )
      ),
      ...
      div(
        ...
        selectInput(
          ...
          selected = c("Agric./Ganad./Pesca", "Industria", "Construcción", "Comercio/Transporte/Hostelería/Info-Com", "Adm./Sanid./Educ./Defen.", "Finanzas/Seguros"),
          multiple = TRUE,
          width = "100%"
        )
      ),
...
observeEvent(input$Indicadort, {
  ...
  if (input$Indicadort == "NACE Rev. 2") {
    updateSelectInput(
      ...
      inputId = "Selec",
      choices = unique(EMPLEO$nace_r2.name),
      selected = c("Agric./Ganad./Pesca", "Industria", "Construcción", 
                  "Comercio/Transporte/Hostelería/Info-Com", 
  ...
  # FORZAR ACTUALIZACIÓN DE REGION cuando cambia el indicador
...
observeEvent(list(input$Region, input$Indicadort), {
  ...
  if (input$Region == "All" && input$Indicadort == "NACE Rev. 2") {
    updateSelectInput(
      session = session,
      inputId = "StaSex",
      choices = setNames(unique(EMPLEO$wstatus), c("Total", "Asalariados", "Autónomos")),
      selected = "EMP"
    )
  }
})
...
renderPlotly({
  ...
  tb_ts <- tb_plot %>%
    as_tsibble(key = c(Sector, Region), index = Año)
  ...
  tb_p <- tb_plot %>%
    as_tsibble(key = c(Sector, wstatus), index = Año) %>%
  ...
  if (input$StaSex != "Ambos") {
  tb_plot <- tb %>% 
    rename(Edad = age, Año = TIME_PERIOD, `Tasa de empleo` =  tasa_emp) %>%
    mutate(`Tasa de empleo` = round(`Tasa de empleo`, 2))
  
  tb_p <- tb_plot %>%
    as_tsibble(key = Edad, index = Año) %>%
  ...
  tb_p <- tb_plot %>%
    as_tsibble(key = c(Edad, sex), index = Año) %>%
...
renderHighchart({
  ...
  if(input$StaSex != "Ambos") tb_base <- tb_base %>% filter(wstatus == input$StaSex)
    ...
    # NUEVO CÓDIGO PARA "All" - Calcular todas las regiones como antes
    list_tb <- list()
    ...
    hchart(
      ...
      hc_yAxis(title = list(text = paste("Valor per cápita -", input$Selec[1]), style = list(fontWeight = "bold"))) %>%
      hc_xAxis(title = list(text = "Año", style = list(fontWeight = "bold"))) %>%
...
if (input$Indicadort == "NACE Rev. 2"){
  ...
  tb_plot <- tb %>% 
    rename(Sector = nace_r2.name) %>%
    filter(Sector != "Total actividades") %>%
    rename(Año = TIME_PERIOD, `Tasa de empleo en edad de trabajar` =  emp_per_working_age)
  ...
  tb_p <- tb_plot %>%
   ...
   hchart(
    ...
    hc_yAxis(title = list(text = "Valor per cápita", style = list(fontWeight = "bold"))) %>%
    hc_xAxis(title = list(text = "Año", style = list(fontWeight = "bold"))) %>%
    hc_tooltip(pointFormat = "{series.name}: <b>{point.y:.3f}</b>") %>%
    hc_legend(enabled = TRUE) %>%
    hc_chart(backgroundColor = "white") %>%
  ...
  tb_plot <- tb %>% 
    rename(Edad = age) %>%
    rename(Año = TIME_PERIOD, `Tasa de empleo` =  tasa_emp)
...
inputs.processing.empleo_region <- reactive({
  ...
  tb <- geoj.tb %>% 
    left_join(
      EMPLEO %>%
      filter(NUTS == input$NUTS, TIME_PERIOD == input$Año, wstatus == "EMP", nace_r2 == "TOTAL"),
      join_by("NUTS_ID" == "geo")
    )
...
inputs.processing.tasa_empleo_gender <- reactive({
  ...
  tb <- geoj.tb %>% 
    left_join(
      GENDER_GAP %>%
      filter(NUTS == input$NUTS_b, TIME_PERIOD == input$Año_b, age == input$Edad),
      join_by("NUTS_ID" == "geo")
    )
...
inputs.processing.key_indicator <- reactive({
  ...
  tb <- geoj.tb %>% 
    left_join(
      KEY_INDICATORS %>%
      filter(Year == input$Año_c, nchar(geo) == 2),
      join_by("NUTS_ID" == "geo")
    )
...
tabla_empleo_regiones <- DT::renderDataTable({
  ...
  req(input$NUTS, input$Año)
  tb <- inputs.processing.empleo_region()
  req(nrow(tb) > 0) 
  
  tb <- tb %>%
    arrange(desc(employment)) %>%
    select(Región = full_name, Valor = employment) %>%
    na.omit() %>%
...
tabla_empleo_sexo_edad <- DT::renderDataTable({
  ...
  req(input$NUTS_b, input$Año_b, input$Edad)
  tb <- inputs.processing.tasa_empleo_gender()
  req(nrow(tb) > 0) 
  
  tb <- tb %>%
   arrange(desc(!!sym(input$Indicador)))%>%  
   select(Región = full_name, Valor = !!sym(input$Indicador)) %>%
   na.omit()
...
tabla_key_indicators <- DT::renderDataTable({
  req(input$Indicador_Key, input$Año_c)
  
  # Filtramos países disponibles
  PAISES <- EMPLEO %>% 
  ...
  tb <- inputs.processing.key_indicator() %>%
    ...
    select(País = Country, Valor = !!sym(input$Indicador_Key), NUTS_ID) %>%
    filter(!is.na(Valor))  # solo eliminar filas sin indicador
  
  # Añadimos tasa de empleo (si hay datos)
  emp_tb <- EMPLEO %>%
    filter(
      TIME_PERIOD == input$Año_c,
      nace_r2 == "TOTAL",
      wstatus == "EMP",
      NUTS == 0
    ) %>%
  ...
  DT::datatable(
    tb %>%
      ...
      select(País, Valor, `Tasa de empleo`),
    options = list(
      paging = FALSE,
      scrollCollapse = TRUE,
      autoWidth = FALSE,
...
mapa_empleo_regiones <- leaflet::renderLeaflet({
  ...
  bindCache(input$NUTS, input$Año)
...
etiquetas <- paste0(
 ...
 # dibujamos el mapa 
 MapaCoroplético(geoj[[as.numeric(input$NUTS_b)+1]], tb[[input$Indicador]], etiquetas, ifelse(input$Indicador == "gender_gap", "Brecha de género", "% de empleo"), palette = paleta, lng  = 24.7038, lat = 50, zoom = 4)
})
...
etiquetas <-paste(
 ...
 # dibujamos el mapa 
 MapaCoroplético(geoj[[1]], tb[[input$Indicador_Key]], etiquetas, "")
})
```
...
tags$style(HTML("
  ...
  /* Color del texto cuando la opción está seleccionada */
...
conditionalPanel(
  ...
  column(
    ...
    wellPanel(
      ...
      "Indicador", 
      label = "Indicador:",
      choices = c("% Mujeres" = "F", "% Hombres" = "M", "% Total" = "T", "Brecha de género (Hombres-Mujeres)" = "gender_gap"),
      selected = "gender_gap"
      ),
...
# En la sección donde defines el observer, modifícalo así:
observeEvent(input$Indicador_Key, {
  req(input$Indicador_Key)
  
  valid_years <- years_key_with_data()
  
  # Solo actualizar si el año actual no está en los años válidos
  current_year <- input$Año_c
  if (!current_year %in% valid_years) {
    ...
    # Mantener el año actual si es válido
    updateSelectInput(
      session,
      "Año_c",
      choices = valid_years,
...
observeEvent(input$y, {
  ...
  current_year <- input$Año.Comparación.Atributos
...
inputs.processing.comparacion.atributos <- reactive( {   
  ...
  if(input$Año.Comparación.Atributos != "All"){
  tb <- tb %>% filter(Year == input$Año.Comparación.Atributos)
}
...
selectInput(
  ...
  label = "Sector:",
  choices = unique(EMPLEO$nace_r2.name), 
  selected = "Comercio/Transporte/Hostelería/Info-Com",
  width = 315
)


sliderInput(
  inputId = "AñoInicialArima",
...
observeEvent(input$región, {

  regiones <- input$región
  ...
  # Sectores disponibles para cada región seleccionada
  ...
  # Intersección de sectores disponibles para TODAS las regiones seleccionadas
  sectores_validos <- Reduce(intersect, sectores_por_region)

  # Manejar casos raros: si la intersección es vacía
...
# Limitar selección a solo 2 regiones
observeEvent(input$región, {

  regiones <- input$región

  if (length(regiones) > 2) {
    # Nos quedamos con las dos últimas seleccionadas
...
```{r}
renderTable({
  regiones_seleccionadas <- head(input$región, 2)
  
  if (length(regiones_seleccionadas) == 0) {
    return(data.frame(Aviso = "Seleccione al menos una región"))
  }
  ...
  tb <- tb %>%
    ...
    mutate(Año=as.character(TIME_PERIOD)) %>%
    relocate(Año) %>%
    select(-TIME_PERIOD)
...
renderPlotly({
 ...
 req(input$región[1], input$AñoInicialArima, input$ArimaParameters, input$sector)
...
tb <- EMPLEO %>%
 ...
 ts <- tb %>%
   filter(full_name == input$región[1], nace_r2.name == input$sector, wstatus == "EMP")  %>%
   mutate(emp_per_working_age = emp_per_working_age * 100) %>%
 ...
 if(input$ArimaParameters == "free"){
   arima <- ts %>%
     filter(TIME_PERIOD >= input$AñoInicialArima) %>%
 ...
 GraficoDinamicoArima95CI(ts, "TIME_PERIOD", "emp_per_working_age", forecast, input$región[1])
...
renderPlotly({
 ...
 req(input$región[2], input$AñoInicialArima, input$ArimaParameters, input$sector)
 ...
 ts <- tb %>%
   filter(full_name == input$región[2], nace_r2.name == input$sector, wstatus == "EMP")  %>%
   mutate(emp_per_working_age = emp_per_working_age * 100) %>%
   as_tsibble(index=TIME_PERIOD) %>%
   fill_gaps()
 ...
 GraficoDinamicoArima95CI(ts, "TIME_PERIOD", "emp_per_working_age", forecast, input$región[2])
...
div(
  ...
  div(
    ...
    tags$label("Año:", style = "margin-bottom: 3px;"),
    selectInput(
      "year_matrix", 
      label = NULL,
...
```{r}
# DIBUJAMOS LA MATRIZ DE CORRELATION USANDO LA FUNCIÓN plot_correlation INCLUIDA EN "utilidades.R"
...
renderPlotly({
  ...
  dplyr::select(-any_of(c("geo", "Country", "Year"))) %>% # eliminamos de la tabla las variables no-numéricas 
  ...
  plot_correlation(show_values = TRUE) 
    ...
    dplyr::select(-geo, -Country, -Year) %>% # eliminamos de la tabla las variables no-numéricas 
    cor(use='complete.obs') %>% # cálculo matriz correlación eliminando NA previamente
    plot_correlation(show_values = TRUE) 
  }
})
...
div(
  ...
  div(
    ...
    selectInput(
      ...
      label = NULL,
      choices = c(
        "Población ocupada (total)"      = "pocu_t",
        "Población ocupada (hombres)"    = "pocu_m",
        "Población ocupada (mujeres)"    = "pocu_f",
        "Población en búsqueda de empleo (total)"    = "psel_t",
        "Población en búsqueda de empleo (hombres)"  = "psel_m",
        "Población en búsqueda de empleo (mujeres)"  = "psel_f",
...
etiquetas <-paste(
 ...
 # dibujamos el mapa 
 map <- MapaCoroplético(municipios.geoj, tb[[ input$indicador_municipio ]],etiquetas,ifelse(substr(input$indicador_municipio, 1, 1) == "p", "Población", "Porcentaje"), lng = -15.5, lat = 28.1, zoom = 8) 
...
### Flujo Edad → Sexo → Tasa de empleo
...
div(
  ...
  div(
    style = "display: flex; flex-direction: column;",
    tags$label("Región", style = "margin-bottom: 5px; font-weight: bold;"),
    selectInput(
  ...
  div(
    style = "display: flex; flex-direction: column;",
    tags$label("Año", style = "margin-bottom: 5px; font-weight: bold;"),
    selectInput(
      inputId = "Año_sankey",
      label = NULL,
...
renderPlotly({
  ...
  # --- Filtrar año ---
  tb <- tb %>% filter(TIME_PERIOD == input$Año_sankey)

  # --- Reescalar la tasa para que el Sankey sea legible ---
  tb <- tb %>% mutate(value = emp_rate)   # puedes dividir entre 1 ó multiplicar si quieres más grosor
  ...
  plot_ly(
    ...
    layout(
      title = paste("Flujo edad → sexo → tasa de empleo (", input$Año_sankey, ")", sep = ""),
      font = list(size = 12),
...
div(
  ...
  div(
    ...
    tags$label("Año: ", style = "margin-right: 5px"),
    selectInput(
      "año_spider",
      label = NULL,
...
```{r}
renderPlotly({
  req(input$año_spider)
  
  datos_año <- INSERCION_LABORAL %>%
    filter(TIME_PERIOD_CODE == as.numeric(input$año_spider)) %>%
  ...
  plot_ly(type = 'scatterpolar') %>%
    ...
    layout(
      ...
      showlegend = TRUE,
      title = list(
        text = paste("<b>Comparativa por Rama de Enseñanza - Año", input$año_spider, "</b>"),
      ...
      margin = list(t = 80, b = 120, l = 80, r = 80)  # Más margen inferior
    )
})
```

### Evolución de contratos por tipo
...
div(
  ...
  div(
    ...
    selectInput(
      "años",
      label = NULL,
      choices = sort(unique(CONTRATOS_2018_2024$año)),
      selected = "2024",
      width = "120px"
    )
  ),
...
```{r}
# Coloca esto AL FINAL de tu dashboard, después de todo el contenido
tags$script(HTML("
  // Función específica para redimensionar Leaflet
    ...
    setTimeout(function() {
      ...
      leafletMaps.forEach(function(mapDiv) {
        ...
        if (map && typeof map.invalidateSize === 'function') {
          // Primero forzar que el contenedor tenga tamaño
          $(mapDiv).css({
            'width': '100%',
            'height': '100%'
          });
  ...
  $(document).ready(function() {
    ...
    // Ajustar cuando cambian las pestañas/tabs
    $('a[data-toggle=\"tab\"]').on('shown.bs.tab', function() {
      setTimeout(forceLeafletResize, 300);
    });
  });
  
  // Observer específico para tu radio button
  ...
  // También disparar cuando Shiny termina de actualizar
